...
function Install-AzureDCAP{
    ...
    $IntelDCAPLocation = (Get-WindowsDriver -Online -All | Where-Object OriginalFileName -Like "*sgx_dcap.inf").OriginalFileName
    $IntelDCAPDir = $IntelDCAPLocation.TrimEnd("\sgx_dcap.inf")
    ...
    Install-ZipTool $PACKAGES["AzureDCAP"]["renamed_file"] `
                    -InstallDirectory $installDir
    $p = Start-Process powershell -Wait -NoNewWindow -PassThru -argument "$installDir\script\InstallAzureDCAP.ps1 $IntelDCAPDir" -WorkingDirectory "$installDir\script"
    if($p.ExitCode -ne 0) {
